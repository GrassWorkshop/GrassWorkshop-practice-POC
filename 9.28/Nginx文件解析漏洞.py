...
def poc(target):
    ...
    try:
        rep = requests.get(target, headers=headers)
        vurl1 = target + '/.php'
        vurl2 = target + '/.xxx'
        vurl3 = target + '/.a'
        ...
        if 'nginx' in str(rep.headers):
            if len(rep1.text) != len(rep2.text) and len(rep1.text) != len(rep3.text) and len(rep2.text) == len(
                    rep3.text):
...
def main():
    parser = argparse.ArgumentParser("Nginx文件解析漏洞检测POC")
